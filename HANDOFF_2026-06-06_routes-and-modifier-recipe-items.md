# 06-06-2026 — Consolidated Routes Handoff (Tool Catalog + Modifier Recipe Items)

> **This is the merge of two handoffs:**
> 1. `05-06-2026 updates and functions of routes.md` — the master tool catalog (all routers) + the 2026-06-05 shopping-list buy-amount update.
> 2. `api-handoff (9).md` — the new **Modifier Recipe Items** feature (2 new routes on the modifier-groups router).
>
> The new routes are now folded into the catalog (Modifier Groups section), with a dedicated deep-dive section preserving the full business logic. **Catalog is now 90 routes across 17 routers.**

> **Frontend: read this differently than a normal handoff.**
> Most handoffs describe one feature, you wire it up once, and you forget it. **This one is a toolbox.** Every route below is a *tool*. You don't use a tool once and discard it — you keep the toolbox, and whenever a new task lands ("show low stock", "let a manager void an order", "redeem a member reward", "make 50% Spicy deduct half the chili"), you come back here, find the tool that does it, and use it.
...
## What changed (the "updates")

### 2026-06-06 — Modifier options can now adjust ingredient deductions (**NEW**)

Each modifier option can now carry a list of **recipe items** — per-ingredient overrides/deltas that change *which ingredients get deducted from stock and by how much* when that modifier is selected. Example: "50% Spicy" overrides chili from 10g → 5g and skips black pepper (`0.000`); "50% Sweet" reduces syrup and adds compensating water via a delta.

- **Two new tools** on the modifier-groups router:
  - `GET /api/v1/modifier-groups/{group_id}/modifiers/{modifier_id}/recipe-items` — list the configured adjustments (BARISTA+).
  - `PUT  /api/v1/modifier-groups/{group_id}/modifiers/{modifier_id}/recipe-items` — bulk-replace them (MANAGER/OWNER).
- **Order flow does not change.** Selecting modifiers when placing an order works exactly as before — the backend applies the deduction math automatically at order time. The only frontend work is in **product/menu management screens**, to let staff *configure* the ingredient adjustments.
- **Full detail** is in the dedicated section [**Modifier Recipe Items — deep dive**](#-modifier-recipe-items--deep-dive-new-2026-06-06) below.

### 2026-06-05 — Shopping List now carries a buy-amount

Previously the shopping list only said *which* ingredient to buy, never *how much*. Now:
...
# The Tool Catalog (all 90 routes)
...
## 🧩 Modifier Groups — options (extra shot, oat milk…) + per-option ingredient adjustments
...
- **`modifier_groups_list_recipe_items`** — `GET /modifier-groups/{group_id}/modifiers/{modifier_id}/recipe-items` · +BARISTA · **When:** configuring a modifier option's ingredient overrides/deltas in the menu-management screen. Returns `[]` when nothing configured (means: use the full base recipe). → `list[ModifierRecipeItemRead]`. *404 if the group isn't in your store or the modifier isn't in that group.* **(NEW 2026-06-06)**
- **`modifier_groups_replace_recipe_items`** — `PUT /modifier-groups/{group_id}/modifiers/{modifier_id}/recipe-items` · +MANAGER · **When:** save the ingredient adjustments for a modifier option. **Full replace** — overwrites all previous entries; send `{"items": []}` to clear. · body `ModifierRecipeItemsBulkReplace` → `list[ModifierRecipeItemRead]` (200). *Idempotent — safe to retry. 422 on bad `mode` or negative `override`. 403 for barista.* **(NEW 2026-06-06)**

> See [**Modifier Recipe Items — deep dive**](#-modifier-recipe-items--deep-dive-new-2026-06-06) for `override`/`delta` semantics, edge cases, and test scenarios.

## 🧾 Orders — the POS checkout + KDS
The heart of the POS floor.

- **`orders_create`** — `POST /orders` · +BARISTA · **When:** checkout. Atomic BOM deduction + idempotency guard. · body `CreateOrderRequest` (incl. `idempotency_key`, items, optional `member_id`/`promotion_ids`/reward) → `OrderRead` (201). *Generate one UUID per checkout, reuse on retry.* Modifier recipe adjustments are applied automatically here — no payload change needed.
...
# 🧩 Modifier Recipe Items — deep dive (NEW 2026-06-06)

## Business context
Modifiers currently only adjust price (e.g. "Small size = -5฿"). This feature lets each modifier option also adjust **which ingredients get deducted from stock** and by how much. For example, "50% Spicy" can reduce chili from 10g to 5g and skip black pepper entirely; "50% Sweet" can reduce syrup and add compensating water. **The backend handles all deduction math automatically at order time** — the frontend only needs to let staff *configure* which ingredients each modifier option affects (in the product/menu-management screens), and *select* modifiers when placing orders as before (no change to the order flow).

## The two tools

### `modifier_groups_list_recipe_items` — `GET /modifier-groups/{group_id}/modifiers/{modifier_id}/recipe-items`
- **Auth:** any authenticated store user (BARISTA and above).
- **Request:** no body — `group_id`, `modifier_id` are path params.
- **Response (200):**
  ```json
  [
  ...
  ```
- **Empty state:** returns `[]` (not 404) when nothing is configured — normal, and means the full base recipe is used when this modifier is selected.
- **Errors:** `404` if `group_id` doesn't belong to the current store, or `modifier_id` doesn't belong to that group.

### `modifier_groups_replace_recipe_items` — `PUT /modifier-groups/{group_id}/modifiers/{modifier_id}/recipe-items`
- **Auth:** MANAGER or OWNER only — barista gets `403`.
- **Request:**
  ```json
  {
    "items": [
      { "inventory_item_id": "inv_chili_id", "quantity": "5.000", "mode": "override" },
      { "inventory_item_id": "inv_water_id", "quantity": "3.000", "mode": "delta" }
    ]
  }
  ```
- **Response (200):** same shape as GET — the full updated list after replacement.
- **Full replace:** every PUT overwrites all previous entries. Do **not** send a partial list expecting a merge. Send `{"items": []}` to clear all adjustments (modifier reverts to base recipe).
- **Idempotent:** retrying the same payload yields the same result — optimistic UI is safe here.
- **Errors:** `404` if group/modifier not found or in another store; `422` on validation failure (bad `mode`, negative `override`, dup `inventory_item_id`).

## Modes — `override` vs `delta`

| Value | Meaning | When to use |
|---|---|---|
| `"override"` | Replace the base recipe quantity with this exact value | Modifier defines a fixed amount (e.g. "50% Spicy = exactly 5g chili"). Set `quantity: "0.000"` to skip deduction entirely (e.g. "0% Spicy = no chili"). Must be `≥ 0`. |
| `"delta"` | Add this value to the base recipe quantity | Modifier adjusts relative to base (e.g. "+3ml water", "−5ml syrup"). Can introduce an ingredient **not** in the base recipe (adds to an implicit base of 0). Negative allowed. |

## Validation rules

| Field | Rule |
|---|---|
| `mode` | Must be `"override"` or `"delta"` — anything else → `422`. Defaults to `"override"` if omitted on input. |
| `quantity` | Decimal **string**, max 10 digits total, 3 decimal places (e.g. `"5.000"`). |
| `quantity` (override) | Must be `≥ 0`. Use `"0.000"` to disable deduction — **not** a negative number. |
| `quantity` (delta) | Can be negative (subtract) or positive (add); range `-999999.999` to `999999.999`. |
| `inventory_item_id` | Must be a valid inventory item in the same store. |
| `items` | Each `inventory_item_id` must be unique within one request (no duplicates per call). |

## Business logic & edge cases
- **Override applies first, then delta.** If two selected modifiers touch the same ingredient (one overrides, one deltas), the override fires first and the delta adjusts from there. *In practice, avoid designing modifier groups where two options can both affect the same ingredient.*
- **Zero-quantity ingredients are not deducted.** If an override resolves to `0`, or a delta drives a quantity negative, no stock movement is created for that ingredient. There is no "negative deduction" — it does nothing.
- **Delta can introduce non-recipe ingredients.** A modifier can add deduction for an ingredient the base product recipe doesn't include (e.g. water for a sweet-drink modifier). Use `delta` — it adds to an implicit base of 0.
- **PRODUCED products are unaffected.** This applies only to `MADE_TO_ORDER` products. `PRODUCED` products deduct finished goods, not recipe ingredients; recipe items are ignored for them.
- **`modifier.inventory_item_id` / `inventory_qty` is a separate, unchanged field.** That field is for purely *additive* modifiers that add an ingredient regardless of the recipe (e.g. "Extra Shot" adds espresso). Don't confuse it with recipe items — different purposes.
- **Shared modifier groups work fine.** A "Sweetness" group linked to multiple products applies the same recipe-item quantities uniformly across all of them. If different products need different absolute amounts for the same concept, they require separate modifier groups.

## Integration notes (config flow in management screens)
1. Load product detail (`products_get`) → expand modifier group → select a modifier option.
2. `modifier_groups_list_recipe_items` → show an ingredient table (name, quantity, mode).
3. Resolve ingredient **names** by joining against `inventory_list` (`GET /inventory`).
4. User edits → `modifier_groups_replace_recipe_items` (PUT) to save the full list.
- **Order placement:** no changes to the order-creation screen or payload — the backend applies adjustments automatically.

## Test scenarios
1. **Happy path — 50% spicy:** PUT `[{chili,"5.000","override"},{pepper,"0.000","override"}]` → 200; subsequent order deducts 5g chili, 0g pepper.
2. **Clear all:** PUT `{"items": []}` → 200 `[]`; subsequent orders use full base recipe.
3. **Delta adds water:** PUT `[{water,"3.000","delta"}]` where water isn't in the recipe → orders deduct 3ml water.
4. **Negative override (invalid):** PUT `{quantity:"-5.000",mode:"override"}` → 422.
5. **Bad mode:** PUT `{mode:"multiply"}` → 422.
6. **Cross-store:** GET/PUT with a `group_id` from another store → 404.
7. **Permission denied:** barista attempts PUT → 403.
...
**Modifier Recipe Items** *(NEW 2026-06-06)*
- `ModifierRecipeItemInput`: `inventory_item_id`, `quantity`(decimal string; negative allowed for `delta` only), `mode`(`"override"` | `"delta"`, defaults to `"override"`)
- `ModifierRecipeItemsBulkReplace`: `items`: [`ModifierRecipeItemInput`] (each `inventory_item_id` unique; `[]` clears)
- *Response* `ModifierRecipeItemRead`: `id`, `inventory_item_id`, `quantity`(string e.g. `"5.000"`), `mode`
...
*Consolidated 2026-06-06 from `05-06-2026 updates and functions of routes.md` + `api-handoff (9).md`. 90 endpoints across 17 routers. Keep this file in sync with the backend — when routes are added, add the tool here, update the count, and keep linking each screen to the tools it uses.*
