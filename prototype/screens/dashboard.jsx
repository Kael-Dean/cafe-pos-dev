// Dashboard — KPI cards, charts (mock), live orders, low stock, staff
  ...
  return (
    <div className="scroll" style={{height: '100%', overflow: 'auto', padding: 24, background: 'var(--color-bg)'}}>
      ...
      <div style={{display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 16}}>
        {d.kpis.map((k) => <AppCommon.KPICard key={k.id} {...k} />)}
      </div>
...
// Mock line chart in pure SVG
const LineChart = ({ hours, today, prev }) => {
  const W = 600, H = 220, P = 28;
  const max = Math.max(...today, ...prev) * 1.1;
  const x = (i) => P + (i * (W - P * 2)) / (hours.length - 1);
  const y = (v) => H - P - (v / max) * (H - P * 2);
  const path = (data) => data.map((v, i) => `${i ? 'L' : 'M'} ${x(i)} ${y(v)}`).join(' ');
  const area = (data) => `${path(data)} L ${x(data.length - 1)} ${H - P} L ${x(0)} ${H - P} Z`;
...
const LiveOrderRow = ({ order }) => {
  ...
  return (
    <div style={{padding: '10px 8px', borderBottom: '1px solid var(--color-surface-2)', display: 'flex', alignItems: 'center', gap: 10}}>
      ...
      <AppCommon.Tag tone={tone}>{label}</AppCommon.Tag>
    </div>
  );
};
  ...
  return (
    <div style={{padding: '10px 8px', borderBottom: '1px solid var(--color-surface-2)'}}>
      <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4}}>
        ...
        <AppCommon.Tag tone={tone}>{Math.round(pct)}%</AppCommon.Tag>
      </div>
...
const StaffRow = ({ staff }) => (
  <div style={{padding: '10px 8px', borderBottom: '1px solid var(--color-surface-2)', display: 'flex', alignItems: 'center', gap: 10}}>
    ...
    <div className="num" style={{fontSize: 13, fontWeight: 600}}>฿{staff.sales.toLocaleString()}</div>
  </div>
);
