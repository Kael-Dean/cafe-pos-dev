// BOM Builder — Recipe management for menu items
// Left: menu list with margin badges. Right: recipe editor + cost/margin summary.
...
const BOMBuilder = () => {
  ...
  // editable copies of recipes + prices
  ...
  const computeCost = (items) => items.reduce((s, r) => {
  ...
  const updateQty = (idx, qty) => setRecipes(r => {
    const next = { ...r };
  ...
  const removeItem = (idx) => setRecipes(r => {
    const next = { ...r };
  ...
  // margin tone helper
  const marginToneOf = (pct) => pct >= 65 ? 'success' : pct >= 50 ? 'warning' : 'danger';
  const marginColorOf = (pct) => {
    if (pct >= 65) return 'var(--color-success)';
    if (pct >= 50) return '#9C6A1F';
    ...
    <div style={{ display: 'flex', height: '100%', background: 'var(--color-bg)' }}>
      ...
      <div style={{
        ...
        <div style={{ padding: 12, borderBottom: '1px solid var(--color-border)', position: 'relative' }}>
          ...
          <input
            type="text" placeholder="ค้นหาเมนู..."
            ...
            return (
              <button key={m.id} onClick={() => setSelectedId(m.id)}
                style={{
                  ...
                  width: '100%', padding: 10, marginBottom: 2, borderRadius: 8,
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ display: 'flex', gap: 6, alignItems: 'center', marginTop: 3 }}>
                    <span className="num" style={{ fontSize: 11, color: 'var(--color-text-secondary)', fontWeight: 600 }}>
                      {AppCommon.baht(mp)}
                    </span>
                    <span style={{ fontSize: 9, color: 'var(--color-text-muted)' }}>•</span>
                    <span className="num" style={{
                      fontSize: 11, fontWeight: 700,
                      color: hasRecipe ? marginColorOf(mar) : 'var(--color-text-muted)',
                    }}>
      ...
      <div className="scroll" style={{ flex: 1, overflow: 'auto', padding: 24 }}>
        ...
        <div style={{
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              <AppCommon.Tag tone="neutral">{cat?.label}</AppCommon.Tag>
              <AppCommon.Tag tone={recipe.length > 0 ? 'success' : 'warning'}>
                {recipe.length > 0 ? `${recipe.length} วัตถุดิบ` : 'ยังไม่มีสูตร'}
              </AppCommon.Tag>
            </div>
          </div>
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
              ...
              <input type="number" min={0} step={5} value={sellingPrice}
                ...
                className="num"
                style={{
                  width: 96, fontSize: 30, fontWeight: 700, textAlign: 'right',
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 16 }}>
          ...
          <SummaryCard label="ส่วนต่าง (Contribution)" value={`฿${margin.toFixed(2)}`}
            color={margin >= 0 ? 'var(--color-text)' : 'var(--color-danger)'} />
          <SummaryCard label="Margin" value={`${marginPct.toFixed(1)}%`}
            highlight={marginToneOf(marginPct)} />
        </div>
          ...
          {recipe.length === 0 ? (
            <div style={{ padding: 48, textAlign: 'center' }}>
              <div style={{
                ...
                <Icon name="inv" size={28} color="var(--color-text-muted)" />
              </div>
              <div style={{ fontSize: 15, fontWeight: 600 }}>ยังไม่มีสูตรสำหรับเมนูนี้</div>
              <div style={{ fontSize: 13, color: 'var(--color-text-secondary)', marginTop: 6, maxWidth: 380, margin: '6px auto 0' }}>
                คลิก "เพิ่มวัตถุดิบ" เพื่อกำหนดสูตร — ระบบจะตัดสต็อกอัตโนมัติเมื่อขายเมนูนี้
              </div>
            </div>
          ) : (
            <>
              <div style={{
                display: 'grid', gridTemplateColumns: '1fr 110px 70px 100px 90px 36px',
                ...
                <div>วัตถุดิบ</div>
                <div style={{ textAlign: 'right' }}>ปริมาณ</div>
                <div>หน่วย</div>
              ...
              {recipe.map((r, idx) => {
                ...
                const stockOk = inv.stock >= r.qty * 10; // arbitrary threshold for demo
                return (
                  <div key={idx} style={{
                    display: 'grid', gridTemplateColumns: '1fr 110px 70px 100px 90px 36px',
                    gap: 12, padding: '12px 20px', alignItems: 'center',
                    borderBottom: idx === recipe.length - 1 ? 'none' : '1px solid var(--color-border)',
                  }}>
                    <div>
                      <div style={{ fontSize: 14, fontWeight: 500 }}>{inv.name}</div>
                      <div style={{ fontSize: 11, color: stockOk ? 'var(--color-text-muted)' : 'var(--color-warning)', marginTop: 2 }}>
                        คงเหลือ {inv.stock.toLocaleString()} {inv.unit}{!stockOk && ' · ใกล้หมด'}
                      </div>
                    </div>
                    <input type="number" step={inv.unit === 'g' || inv.unit === 'ml' ? 1 : 0.5} min={0}
                      value={r.qty}
              ...
              <div style={{
                padding: '14px 20px',
                display: 'grid', gridTemplateColumns: '1fr 90px 36px', gap: 12,
                alignItems: 'center',
                background: 'var(--color-surface-2)',
                borderTop: '2px solid var(--color-border)',
              }}>
                <div style={{ fontSize: 13, fontWeight: 700 }}>ต้นทุนรวมต่อหน่วยขาย</div>
                <div className="num" style={{ fontSize: 16, fontWeight: 800, textAlign: 'right' }}>
                  ฿{totalCost.toFixed(2)}
                </div>
                <div></div>
              </div>
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 16, justifyContent: 'flex-end' }}>
          <button
            onClick={() => toast({
              ...
              msg: `${menu.name} • ${recipe.length} วัตถุดิบ • ต้นทุน ฿${totalCost.toFixed(2)} • Margin ${marginPct.toFixed(1)}%`,
            })}
            style={{
        ...
        <div style={{
          ...
          background: 'var(--color-info-50)', borderRadius: 12,
          ...
          <div>
            <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--color-info)', marginBottom: 6 }}>
              เรื่อง Margin ที่ควรรู้
            </div>
            <div style={{ fontSize: 12, color: 'var(--color-text-secondary)', lineHeight: 1.7 }}>
              คาเฟ่ทั่วไปควรมี margin <strong style={{ color: 'var(--color-success)' }}>≥ 65%</strong> สำหรับเครื่องดื่ม และ <strong style={{ color: 'var(--color-success)' }}>≥ 60%</strong> สำหรับเบเกอรี่ เพื่อครอบคลุมต้นทุนคงที่ (ค่าเช่า ค่าจ้าง ค่าน้ำไฟ) · หาก margin ต่ำกว่า <strong style={{ color: 'var(--color-danger)' }}>50%</strong> ให้พิจารณาขึ้นราคา ลด portion หรือเปลี่ยนซัพพลายเออร์ · ระบบจะตัดสต็อกตามสูตรนี้อัตโนมัติทุกครั้งที่ขาย
            </div>
          </div>
        </div>
      </div>
      ...
      {picker && <IngredientPicker
        existingIds={recipe.map(r => r.invId)}
        onSelect={addItem}
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        color: t ? t.fg : 'var(--color-text-secondary)',
      }}>{label}</div>
      <div className="num" style={{
        fontSize: 22, fontWeight: 700, letterSpacing: '-0.02em',
        color: t ? t.fg : (color || 'var(--color-text)'),
      }}>{value}</div>
    </div>
  );
};
...
// ─── Ingredient picker modal ───
const IngredientPicker = ({ existingIds, onSelect, onClose }) => {
  const [q, setQ] = useState('');
  const list = INVENTORY.filter(inv =>
    !q || inv.name.toLowerCase().includes(q.toLowerCase())
  );
  return (
    <div style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ padding: 20, borderBottom: '1px solid var(--color-border)' }}>
          ...
          <div style={{ position: 'relative' }}>
            ...
            <input type="text" placeholder="ค้นหาวัตถุดิบ..." autoFocus
        ...
        <div className="scroll" style={{ overflow: 'auto', flex: 1, padding: 8 }}>
          {list.length === 0 ? (
            <div style={{ padding: 40, textAlign: 'center', color: 'var(--color-text-muted)', fontSize: 13 }}>
              ไม่พบวัตถุดิบที่ตรงกับ "{q}"
            </div>
          ) : list.map(inv => {
            const exists = existingIds.includes(inv.id);
            return (
              <button key={inv.id} onClick={() => !exists && onSelect(inv.id)}
                disabled={exists}
                style={{
                  display: 'flex', gap: 12, alignItems: 'center',
                  width: '100%', padding: 12, marginBottom: 2, borderRadius: 8,
                ...
                onMouseEnter={e => { if (!exists) e.currentTarget.style.background = 'var(--color-surface-2)'; }}
                onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; }}
              >
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 14, fontWeight: 600 }}>{inv.name}</div>
                  <div className="num" style={{ fontSize: 12, color: 'var(--color-text-secondary)', marginTop: 2 }}>
                    ฿{inv.costPerUnit.toFixed(2)}/{inv.unit} · คงเหลือ {inv.stock.toLocaleString()} {inv.unit}
                  </div>
                </div>
                {exists
                  ? <AppCommon.Tag tone="success">เพิ่มแล้ว</AppCommon.Tag>
