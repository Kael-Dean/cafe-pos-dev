// POS Terminal screen — Menu grid + Cart + Modifier modal + Payment QR modal
// Implements Flow A: order → modify → cart → QR → success
...
const POSTerminal = () => {
  ...
  const [modifierItem, setModifierItem] = useState(null); // menu item being modified
  const [editingCartIndex, setEditingCartIndex] = useState(null);
  const [payment, setPayment] = useState(null); // 'cash'|'card'|'qr'|'line'
  ...
  const onMenuClick = (item) => {
    if (item.needsModifier || item.cat === 'hot' || item.cat === 'cold' || item.cat === 'tea') {
      // For drinks, open modifier modal
      if (item.cat === 'hot' || item.cat === 'cold' || item.cat === 'tea') {
        setModifierItem({ item, mode: 'add' });
        return;
      }
    }
    // No modifier — add directly
    addLine({
      menuId: item.id,
      name: item.name,
  ...
  const addLine = (line) => {
    setCart((cur) => {
      // try to merge identical (same menu + same modKey)
  ...
  const removeLine = (i) => setCart((cur) => cur.filter((_, k) => k !== i));

  const clearCart = () => { setCart([]); setBillNo((b) => b + 1); };

  const onPaid = () => {
    setPayment(null);
    toast({ kind: 'success', title: 'ชำระเงินสำเร็จ', msg: `บิล A0${billNo} • ${AppCommon.baht(total)} • ส่งครัวแล้ว`, duration: 3500 });
    clearCart();
  };
    ...
    <div style={{display: 'flex', height: '100%', overflow: 'hidden', background: 'var(--color-bg)'}}>
        ...
        <div style={{padding: '16px 20px 0 20px', display: 'flex', flexDirection: 'column', gap: 12}}>
          <div style={{display: 'flex', gap: 12, alignItems: 'center'}}>
            ...
            <div style={{flex: 1, position: 'relative'}}>
              ...
              <input
                type="text"
                placeholder="ค้นหาเมนู ชื่อ หรือ hotkey..."
                value={search}
          ...
          <div style={{display: 'flex', gap: 6, overflowX: 'auto'}} className="scroll">
            <CategoryTab id="fav" label="★ ขายดี" active={category === 'fav'} onClick={() => { setCategory('fav'); setSearch(''); }} highlight />
            {CATEGORIES.filter((c) => c.id !== 'fav').map((c) => (
              <CategoryTab key={c.id} id={c.id} label={c.label} active={category === c.id} onClick={() => { setCategory(c.id); setSearch(''); }} />
            ))}
          </div>
        </div>
      ...
      <div style={{flex: '0 0 40%', display: 'flex', flexDirection: 'column', background: 'var(--color-surface)'}}>
        ...
        <div style={{padding: 20, borderTop: '1px solid var(--color-border)', background: 'var(--color-surface-2)'}}>
          <Row label="Subtotal" value={AppCommon.baht(subtotal)} />
          <Row label="VAT 7%"  value={AppCommon.baht(vat)} />
          <Row label="ส่วนลด"  value={AppCommon.baht(0)} muted />
      ...
      {modifierItem && (
        <ModifierModal
          ...
          onAdd={(line) => {
            addLine(line);
            toast({ kind: 'success', title: 'เพิ่มลงตะกร้า', msg: `${line.name} • ${AppCommon.baht(line.unitPrice)}`, duration: 1800 });
...
// ---- Subcomponents ----
const CategoryTab = ({ label, active, onClick, highlight }) => (
  <button
    onClick={onClick}
    style={{
...
const MenuCard = ({ item, onClick }) => (
  ...
  >
    {/* Image placeholder using item color */}
    <div style={{
      aspectRatio: '4 / 3',
...
const CartLine = ({ line, onInc, onDec, onRemove }) => (
...
const PayButton = ({ icon, label, onClick, disabled, primary }) => (
