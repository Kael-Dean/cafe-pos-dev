...
import { KDS_TICKETS } from '../data/mock-data';

export default function KDS() {
  const toast = useToast();
  const [tickets, setTickets] = useState(() => KDS_TICKETS.map(t => ({ ...t })));
...
const OrderTicket = ({ ticket, mins, onBump, onDone }: { ticket: typeof KDS_TICKETS[0]; mins: number; onBump: () => void; onDone: () => void }) => {
  ...
  return (
    <div style={{
      background: 'white', borderRadius: 12, borderTop: `4px solid ${accent}`,
      color: 'var(--color-text)', display: 'flex', flexDirection: 'column', overflow: 'hidden',
